...
char *notfound_html = "<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML "
                      "2.0//EN\"><html><body>Not Found</body></html>";
char *badrequest_html = "<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML "
                        "2.0//EN\"><html><body>Bad Request</body></html>";
...
void create_time_line(char *buffer) {
  ...
  snprintf(buffer, 64, "Date: %.3s, %d %.3s %d %.2d:%.2d:%.2d GMT",
           wday_name[ptm->tm_wday], ptm->tm_mday, mon_name[ptm->tm_mon],
...
int vcomms_received(struct CliConn *client, char *message,
  ...
  if (reqtype == REQ_GET) {
    ...
    if (vcomms_parseparam(client, par) == 0) {
      ...
      toLog(0, "SSE connection: OK <%d>\n", client->descr);
      toLog(0, ">>>>>> start data stream <<<<<<\n");
      ++(pvisstat->allclient);
    } else {
      toLog(0,
            "Error in GET parameters. <%d> (Missing or wrong \"subscribe\" "
            "parameter)\n"
            "Closing connection...\n",
            client->descr);
      vcomms_send_badreq(client);
  ...
  if (reqtype == REQ_HEAD) {
    if (urlmatch != URL_YES) {
      toLog(0, "Client request unknown URI, Closing <%d>...\n", client->descr);
      vcomms_send_notfound(client);
      close_client(client->descr);
      return 0;
    }
...
int vcomms_payload_encode(char *outbuffer, char *message) {
  chop(message);
  snprintf(vcomms_chunk_encoding_idbuffer, 60,
           "id: %ld\ndata: ", (long)time(NULL));
  snprintf(outbuffer, SSE_PAYLOADMAX, "%s%s\n\n\r\n",
           vcomms_chunk_encoding_idbuffer, message);
  return 0;
}
...
int vcomms_chunk_encoding(char *outbuffer, char *message) {
  ...
  snprintf(outbuffer, SSE_PAYLOADMAX, "%x\r\n%s%s\n\n\r\n",
           (unsigned int)(strlen(vcomms_chunk_encoding_idbuffer) +
                          strlen(message) + 2), //+2 is the two \n
           vcomms_chunk_encoding_idbuffer, message);
  return 0;
}
...
int vcomms_send_badreq(struct CliConn *client) {
  ...
  snprintf(buffer, 1024,
           ...
           tbuff, pvisset->service, (int)strlen(badrequest_html));
  cio_high_write(client, buffer);
...
int vcomms_send_notfound(struct CliConn *client) {
  ...
  snprintf(buffer, 1024,
           ...
           "Connection: close\r\n"
           "Content-Type: text/html; charset=utf8\r\n\r\n",
           tbuff, pvisset->service, (int)strlen(notfound_html));
  cio_high_write(client, buffer);
