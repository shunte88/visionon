...
static void vissy_reopen(void) {
  ...
  */
  ...
  if (vis_fd > 0) {
    // Map memory.
    vis_mmap = mmap(NULL, sizeof(struct vis_t), PROT_READ | PROT_WRITE,
                    MAP_SHARED, vis_fd, 0);
...
void vissy_meter_init(struct vissy_meter_t *vissy_meter) {
  ...
  //			printf("subband: %d, decade_idx: %d, decade_len: %d,
  //preemphasis: %f\n", 			s, vissy_meter->decade_idx[s],
  //vissy_meter->decade_len[s], vissy_meter->preemphasis[s]);
  //		}
}
...
bool stashvissy_meter_calc(struct vissy_meter_t *vissy_meter) {
    ...
    for (i = 0; i < num_samples; i++) {
      for (channel = 0; channel < METER_CHANNELS; channel++) {
        ...
        /*
                                        if (0==channel) {
                                                fin_buf[i].r = (float)
           (vissy_meter->filter_window[i] * sample); } else { fin_buf[i].i =
  ...
  /*
          ...
          for( s = 0; s < vissy_meter->num_subbands; s++) {
                  ...
                  for( x = vissy_meter->decade_idx[s]; x <
     vissy_meter->decade_idx[s] + vissy_meter->decade_len[s]; x ++) { ck =
     fout_buf[x]; cnk = fout_buf[vissy_meter->sample_window - x];

                          kr = ( ck.r + cnk.r) / 2;
                          ki = ( ck.i - cnk.i) / 2;

                          vissy_meter->avg_power[avg_ptr] += ( kr * kr + ki *
     ki) / vissy_meter->num_windows;

                          kr = ( cnk.i + ck.i) / 2;
                          ki = ( cnk.r - ck.r) / 2;

                          vissy_meter->avg_power[avg_ptr+1] += ( kr * kr + ki *
     ki) / vissy_meter->num_windows;
                  }

                  vissy_meter->avg_power[avg_ptr] /= vissy_meter->decade_len[s];
                  vissy_meter->avg_power[avg_ptr+1] /=
     vissy_meter->decade_len[s]; avg_ptr += 2;
...

          {
                  ...
                  for( p = 0; p < vissy_meter->num_subbands; p++) {
                          ...
                          long product = (long) (
     vissy_meter->avg_power[avg_ptr] * vissy_meter->preemphasis[pre_ptr]);
  ...
  */
  for (channel = 0; channel < METER_CHANNELS; channel++) {
    ...
    /*		int power_sum = 0;
                    ...
                    for( s = 0; s < vissy_meter->num_subbands; s++) {
                            // Average out the power for all subbands
       represented
                            // by a bar.
       ...
       vissy_meter->subbands_in_bar[channel];
                            ...
                            if( ++in_bar ==
       vissy_meter->subbands_in_bar[channel]) { int val; int i;
                                    ...
                                    for( i = 31; i > 0; i--) {
                                            if( power_sum >=
       vissy_meter->power_map[i]) { val = i; break;
                                            }
                                    }

                                    vissy_meter->sample_bin_chan[channel][curr_bar++]
       = val;

                                    if( curr_bar ==
       vissy_meter->num_bars[channel]) { break;
                                    }
    ...
    if (samode) // SA mode
    {
      ...
      for (int s = 0; s < vissy_meter->num_subbands; s++) {
        ...
        for (x = vissy_meter->decade_idx[s];
             x < vissy_meter->decade_idx[s] + vissy_meter->decade_len[s]; x++) {
          ck = fout_buf[x];
