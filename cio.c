...
int cio_init(bool use_ssl, const char *certfile, const char *pkeyfile) {
  if (cio_high_read_callback == NULL || cio_low_write_callback == NULL) {
    printf("SSL Error, you must set cio_high_read_callback and "
           "cio_low_write_callback functions!\n");
    return 1;
  }
  ...
  if (no_ssl) {
    toLog(0, "cio_init no SSL.\n");
    return 0;
  } else
    toLog(0, "Initialize SSL.\n");
...
int cio_low_read(struct CliConn *client, char *buffer, int length) {
  ...
  if (written > 0 || 1) {
    if (!SSL_is_init_finished(CIO(client)->ssl)) {
      ...
      if (rv == 1) {
        printf("SSL_accept() succesful, ssl connection established.\n");
        client->status = STATUS_NEW;
        write_outbio_pending_to_low(client);
      }
      if (rv == 0) {
        rv2 = SSL_get_error(CIO(client)->ssl, rv);
        printf("SSL_accept ERROR, Connection shut down. (0):\nSSL_get_error "
               "says %d, %s\n",
      ...
      if (rv < 0) {
        ...
        if (rv2 == SSL_ERROR_WANT_READ || rv2 == SSL_ERROR_WANT_WRITE)
          ...
          printf("SSL_accept returned <0 (%d):\nSSL_get_error says %d, %s\n",
  ...
  if (SSL_is_init_finished(CIO(client)->ssl)) {
    ...
    if (read == 0) {
      ...
      printf("SSL_read returns 0, Error: %d, %s\n", rv2,
             cio_ssl_error_text(rv2));
    }
    if (read < 0) {
      rv2 = SSL_get_error(CIO(client)->ssl, read);
      if (rv2 == SSL_ERROR_WANT_READ || rv2 == SSL_ERROR_WANT_WRITE)
...
int cio_client_close(struct CliConn *client) {
  ...
  if (client->cio != NULL) {
    ...
    printf("SSL: Shutdown/Free connection.\n");
    client->cio = NULL;
  }
  return 0;
}
