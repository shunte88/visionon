...
static const char client_status_name[][5] = {"UNK", "NEW", "SSL",
                                             "HEA", "SSE", "END"};
...
struct CliConn *client_add(int descr) {
  ...
  if (bCurr->next <= bCurr->max) {
    ...
    bCurr->n->clis =
        (struct CliConn *)malloc(sizeof(struct CliConn) * BANKSIZE);
    bCurr->n->next = 1;
...
int client_del(int descr) {
  ...
  // If we deleted the last item in a bank need to delete the last (now enpty)
  // bank
...
void client_list(int level) {
  ...
  while ((c = client_next()) != NULL) {
    ...
    printf("#%d - <%d> info: %s S:%s Err: %d Messages: %d Reinit: %d SSL:%s\n",
           i, c->descr, c->info, client_status_name[c->status], c->err,
           c->message, c->reinit, (c->cio == NULL ? "No" : "Yes"));
    printf("  Connection time: %s  Handshaked time: %s UniqId:%s\n", dt2, dt1,
           c->uniq_id);
    if (strlen(c->agent) > 0)
      printf("  Agent: %s\n", c->agent);
...
int client_subscribe_exists(struct CliConn *cli, char *ss, int mmatch,
                            char *rejectId) {
